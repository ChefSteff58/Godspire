...
function Pip({ value, label, gold }: { value: string; label: string; gold?: boolean }) {
  return (
    <div className="text-center">
      <div className={`font-pixel text-xl font-bold ${gold ? 'text-amber-300' : 'text-slate-100'}`}>{value}</div>
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col gap-0.5">
        <span className="flex items-center justify-between gap-2">
          <span className="font-pixel text-xs font-bold text-slate-100">{node.name}</span>
          <span className={`font-pixel shrink-0 text-[11px] font-bold ${owned ? 'text-amber-300' : 'text-slate-300'}`}>
            {owned ? '✓' : `◆ ${node.cost}`}
          </span>
        </span>
